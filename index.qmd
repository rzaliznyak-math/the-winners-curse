...
> Among simulated experiments that are _statistically significant_, what fraction of observed effects exceed the true effect?

$$
\Pr\!\left(\widehat{\Delta} > \Delta_{\text{true}} \mid \text{Statistical Significance}\right)
$$

where $\widehat{\Delta}$ denotes the observed index-to-control from the experiment,
and $\Delta_{\text{true}}$ denotes the true underlying effect.
...
- `{python} f"{pr_over_estimate_statssig:.1%}"` of our simulated experiments that reached _Stats Sig_ overestimated the true effect we programmed into our simulations.
...
- Because the alternative distribution of the estimator is centered at the true
  effect, the observed effect is equally likely to fall above or below the true
  effect:
  $$
  \Pr(\widehat{\Delta} > \Delta_{\text{true}}) = 0.5
  $$

- Because the true effect is present, the probability that the experiment reaches
...
significant experiments overestimated the true effect.

## Increase Power

You can increase your statistical power to reduce the chance of effect size inflation. But the problem never truly vanishes.
...
fig = px.line(x=rho, y=f_rho, 
              #title="S(ρ) = ρ²",
              labels={'x': 'power', 'y': 'prct_over_estimate'},
...
- Even as power increases toward 100%, the chance of overestimating the true effect never falls below 50%.
...
Statistical significance selects for extreme outcomes.

Conditioning on statistical significance selects only experiments whose observed effects crossed the significance threshold. This creates an upward bias in observed effect sizes — **The Winner’s Curse**.

Even with a properly designed experiment at 80% power, a statistically significant result is more likely to overestimate than underestimate the true index-to-control --- 62.5% of the time.

Statistical significance provides evidence that an effect exists, not that the observed effect size is accurate.