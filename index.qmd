...
You did everything correctly with your AB Test --- determined your required sample size in advance, collected all of your samples, and calculated a p-value < 0.01 at the end of your experiment.

But _Significance Testing_ is not an endorsement of the observed treatment effect  --- even with a low p-value! 
Walk through the below `numpy.random` enabled simulations or repeat on your own to see why this is the case.
...
We set MDE = 101 (index-to-control), representing a +1% relative lift, a typical effect size that product experiments in industry are powered to detect.
...
> **Note**: An MDE (index-to-control) of 101 corresponds to a +1% relative change --- in this case going from 65% to 65.65% --- a delta-to-control of 0.65%.
...
Effect size inflation is unavoidable in low and moderate powered experiments. Practitioners have two options: increase statistical power to reduce the magnitude of the bias, or use priors to produce more stable effect size estimates across repeated experiments.

## Increase Statistical Power

You can increase your statistical power to reduce the chance of effect size inflation. But the problem of over-estimates never truly vanishes.
...
- The share of overestimates reaches its minimum of 50% at 100% power.


## Use Priors

If you want good long-run effect size estimates, priors are required.

Observed results are weighed against the historical prior using precision-weighted averaging, with more confident (lower-variance) inputs exerting greater influence.

```{python}
#| code-summary: "<b>Method to Regularize Priors</b>"
#| echo: true
#| code-fold: true
...
Before you looked at the current data, you already had beliefs — based on experience, past experiments, or domain expertise. That’s your prior.

But you’ve collected new data — this is the observed result. Your brain combines what you previously believed (the prior) with what you just learned (the observed data) to produce a new estimate: the posterior.

Your point of view evolves with new evidence and your estimate shifts as new data tells a stronger or different story.

# Conclusion

Statistical significance conditions on extreme results, selecting only experiments whose observed effects cross the significance threshold and are thus biased to overestimate effect sizes — **the Winner’s Curse**.

We showed that even in a properly designed experiment with 80% statistical power, a statistically significant result is more likely to overestimate than underestimate the true index-to-control — 62.5% of the time.

**In short**: statistical significance provides evidence that an effect exists, not that the observed effect size is accurate.